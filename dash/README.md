...
| Thing | Value |
|---|---|
| Study site | `https://dash.study.childmind.org` |
| Prolific study URL | `https://dash.study.childmind.org/start` — see the note below |
| Opt-in form endpoint | `https://dash.study.childmind.org/api/opt-in` |
...
TOKEN=$(ssh arno@167.71.248.46 \
    ...
    | /usr/bin/tr -d ' \t\r\n')
curl -s "https://dash.study.childmind.org/admin/linkage.csv?token=$TOKEN" \
    -o ~/Desktop/linkage-$(date +%F).csv
```

`167.71.248.46` is the droplet; `dig +short dash.study.childmind.org` confirms
